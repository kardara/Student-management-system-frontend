...
export default function ManageApplications() {
  ...
  const applyFilter = (a) => {
    ...
    if (searchString && statusFilter && departmentFilter) {
      return a.filter(
        (e) =>
          ...
          departmentFilter.toLowerCase() == e.department.code.toLowerCase()
      );
    }
    ...
    if (departmentFilter) {
      return a.filter(
        (e) => departmentFilter.toLowerCase() == e.department.code.toLowerCase()
      );
    }

    if (statusFilter) {
      return a.filter(
        (e) => statusFilter.toLowerCase() == e.status.toLowerCase()
      );
    }
    if (searchString) {
                ...
                <tbody>
                  {currentItems.map((item, id) => (
                    ...
                    >
                      ...
                      >
                        {" "}
                        <span className="uppercase cursor-pointer text-sm p-2 font-black text-accent-light dark:text-accent-dark">
                          Aprove
                        </span>{" "}
                      </td>
                      <td
