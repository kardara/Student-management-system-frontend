...
export default function ManageGrade() {
  ...
  const headers = ["S/N", "Code", "Course name", "Group", "Lecturer"]

  return (
    <>
      <div className='flex flex-col' style={{ display: showAddModal || showViewModal || showEditModal ? "none" : "flex" }} >
        <ManagementComponentHeader title="Grades " setShowAddModal={setShowAddModal} />
        <div>
        </div>
        <div>
              ...
              <table >
                ...
                <tbody>
                  {currentItems.map((item, id) => (
                    <tr key={id}
                      onClickCapture={() => { navigate(`course/${item.id}`);setCourseModalId(item.id); setShowEditModal(false); setShowViewModal(true) }}
