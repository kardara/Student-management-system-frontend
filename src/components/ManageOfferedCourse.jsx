...
export default function ManageOfferedCourse() {
  ...
  const headers = ["S/N", "Code", "Course name", "Group", "Day", "Time", "Credit", "Size", "Room", "Lecturer"]

  return (
    <>
      <div className='flex flex-col' style={{ display: showAddModal || showViewModal || showEditModal ? "none" : "flex" }} >
        <ManagementComponentHeader title="Offered Courses " name="Add Course" setShowAddModal={setShowAddModal} />
              ...
              <table >
                ...
                <tbody>
                  {currentItems.map((item, id) => (
                    <tr key={id}
                      onClickCapture={() => { setCourseModalId(item.id); setShowEditModal(false); setShowViewModal(true) }}
      ...
      <AddOfferedCourseModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} />
