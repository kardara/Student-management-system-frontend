...
import { User, Users, Mail, Phone, MapPin, GraduationCap, BookOpen } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export default function RenderTeacherSearch({ data }) {
  ...
  pathname.splice(2);
  pathname.push("teachers/view/");

  if (!data?.length || !data) {
    return (
      <div className="rounded-lg shadow-md p-6 bg-white dark:bg-gray-800" key={"teachers"}>
        <div className="flex items-center gap-2 mb-4 border-b pb-2 text-gray-700 dark:text-gray-200">
          <GraduationCap size={20} />
          <h5 className="text-lg font-semibold">Teachers</h5>
        </div>
        <div className="flex flex-col items-center justify-center py-8 text-center opacity-40">
          <User size={32} className="mb-3 opacity-30" />
          <p className="text-base">No teachers found</p>
        </div>
      </div>
    );
  }

  // Sort teachers by firstName for better organization
  const sortedTeachers = [...data].sort((a, b) =>
    a.firstName.localeCompare(b.firstName) || a.lastName.localeCompare(b.lastName)
  );

  return (
    <div className="rounded-lg shadow-md p-6 mb-5 bg-white dark:bg-gray-800" key={"teachers"}>
      <div className="flex items-center gap-2 mb-4 border-b-2 border-gray-100 dark:border-gray-700 pb-2">
        ...
        <h5 className="text-lg font-semibold text-gray-800 dark:text-white">Teachers</h5>
      ...
      <div className="ml-5 divide-y divide-gray-100 dark:divide-gray-700">
        {sortedTeachers.map((teacher) => (
          <div
            ...
            onClick={() => navigate(pathname.join("/") + teacher.id, )}
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4 flex-grow">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 flex items-center">
                  ...
                  <span className="mr-2">{teacher.qualification || 'No Qualification'}</span>
                </div>
              </div>
            </div>
