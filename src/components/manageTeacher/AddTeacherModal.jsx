...
export default function AddTeacherModal({ isOpen, onClose }) {
    ...
    const addTeacher = () => {
        ...
        const teacher = { firstName, lastName, qualification, role, email, phone, address }

        console.log(teacher);


        manageTeachersService.add(teacher, parent)
            .then((resp) => {
                toast.success(resp, { theme: localStorage.getItem('theme'), position: "top-center" });
                setTimeout(() => window.location.reload(), 750);
                onClose()
            },
                (err) => {toast.error(err, { theme: localStorage.getItem('theme'), position: "top-center" }); console.log(err);
                })
    ...
    return (
        <div>
            ...
            <div className=' bg-bg-secondary-light dark:bg-bg-secondary-dark mt-5 px-5 py-10 rounded-2xl max-w-md mx-auto  '>
                <div>
                    <div>
                        ...
                        <div className='flex flex-col mt-4'>
                            <SelectTeacherQualification qualification={qualification} onChange={e => setQualification(e.target.value)} />
                        </div>

                        <div className='flex flex-col mt-4'>
                            <SelectTeacherRole role={role} onChange={e => setRole(e.target.value)} />
                        </div>
                        ...
                        <div className='flex flex-col mt-4'>
                            <InputField value={phone} onChange={(e) => setPhone(e.target.value)} name="Phone" />
                        </div>
            ...
            <div className='flex gap-5 justify-center mt-5'>
                ...
                <Button name="Add" onClick={() => addTeacher()}>ADD</Button>
            </div>
        </div>
